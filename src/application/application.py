...
class Application:
    ...
    def __init__(self, reader, data_frame_transformer, row_dispatcher, column_statistics_calculator, results_viewer):
        self._reader = reader
    ...
    def run(self):
        data_frame = self._reader.read(filename=self.SOURCE_DATASET_PATH,
                                       separator=": ",
                                       header=False)
        transformed_data_frame: DataFrame = self._data_frame_transformer.transform(data_frame)
        dataset_rdd_cached = transformed_data_frame.rdd.cache()
        results = self._row_dispatcher.dispatch(dataset_rdd_cached, len(transformed_data_frame.columns))
