...
class StringProcessor(Processor):
    ...
    def process(self, column_rdd):
        ...
        characters_rdd_cached = key_message_rdd_cached.flatMap(self._character_split).cache()
        count_distinct_characters, count_distinct_strings, count_distinct_words = self._get_count_distincts(
            key_message_rdd_cached, words_rdd_cached, characters_rdd_cached)
        return StringResults(message_length_statistics=message_length_statistics,
                             ...
                             messages_entropy=self._column_statistics_calculator.calculate_entropy(key_message_rdd_cached),
